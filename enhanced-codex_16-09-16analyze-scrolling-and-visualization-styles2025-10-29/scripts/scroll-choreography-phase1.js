(function () {
  ...
  let detachVisibilityLifecycle = null;

  const reduceMotionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');

  const createVisibilityLifecycle = ({ onHidden, onVisible } = {}) => {
    ...
    const handleChange = () => {
      ...
      } else if (typeof onVisible === 'function') {
        onVisible();
      }
    };
  ...
  const handleReduceMotion = () => {
    if (reduceMotionQuery.matches) {
      ...
      if (typeof detachVisibilityLifecycle === 'function') {
        detachVisibilityLifecycle();
        detachVisibilityLifecycle = null;
      }
  ...
  function initialiseChoreography() {
    ...
    if (window.Lenis) {
      ...
      const runLenisFrame = (time) => {
        if (!lenisInstance || lenisSuspended) {
          lenisFrame = null;
          return;
        }
      ...
      const startLenisFrame = () => {
        if (!lenisInstance || lenisSuspended || lenisFrame) {
          return;
        }
        lenisFrame = requestAnimationFrame(runLenisFrame);
      };
    ...
    releaseSchedulerState = () => {
      ...
      pointerState.active = false;
      pointerState.shiftX = 0;
      pointerState.tiltX = 0;
      pointerState.tiltY = 0;
      pointerState.focus = 0;
      heroFocusProxy.base = 0;
      heroFocusProxy.pointer = 0;
      heroFocusCommit.active = false;
      heroFocusCommit.lastBase = NaN;
      heroFocusCommit.lastPointer = NaN;
      heroFocusCommit.lastTotal = NaN;
      moireProxy.baseShift = 0;
      moireProxy.pointerShift = 0;
      moireProxy.rotation = 0;
      moireProxy.scale = 1;
      ...
      moireProxy.driftY = 0;
      moireCommit.active = false;
      moireCommit.last.shift = '';
      moireCommit.last.rotation = '';
      moireCommit.last.scale = '';
      moireCommit.last.opacity = '';
      moireCommit.last.driftX = '';
      moireCommit.last.driftY = '';
      pointerTarget.shiftX = 0;
      pointerTarget.tiltX = 0;
      pointerTarget.tiltY = 0;
      pointerTarget.focus = 0;
    ...
    detachVisibilityLifecycle = createVisibilityLifecycle({
      onHidden() {
        pauseLenis();
        frameScheduler.clear();
        if (typeof releaseSchedulerState === 'function') {
        ...
        heroBridgeUpdate({ focus: 0, pointerFocus: 0, pointerShift: 0 });
      },
      onVisible() {
        resumeLenis();
