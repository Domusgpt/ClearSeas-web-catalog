...
export class VisualOrchestrator {
    ...
    broadcastState(deltaTime = 16) {
        ...
        const significantChange = this.hasSignificantBroadcastChange(payload);

        const meetsInterval = control.elapsed >= control.interval;
        ...
        if (!significantChange) {
            if (!needsIdleUpdate) {
                ...
                control.elapsed = Math.min(control.elapsed, control.interval);
                return;
            }
            if (!meetsInterval) {
                return;
    ...
    hasSignificantBroadcastChange(payload) {
        const control = this.broadcastControl;
        ...
        const differs = (next = {}, prev = {}, thresholds = {}) => {
            return Object.keys(next).some((key) => {
                ...
                if (typeof nextValue === 'number' && typeof prevValue === 'number') {
                    ...
                    return Math.abs(nextValue - prevValue) > epsilon;
                }
        ...
        if (differs(payload.state, last.state, control.thresholds.state)) {
            return true;
        }

        if (differs(payload.multipliers, last.multipliers, control.thresholds.multipliers)) {
            return true;
        }

        if (differs(payload.context, last.context, control.thresholds.context)) {
            return true;
        }
