...
export class VisualOrchestrator {
    constructor(canvasManager) {
        ...
        this.broadcastControl = {
            ...
            thresholds: {
                ...
                context: {
                    ...
                    hoveredCards: 1,
                    sectionHue: 0.6,
                    sectionSaturation: 0.6,
                    sectionLightness: 0.6,
                    sectionHighlight: 0.6,
                    sectionShadow: 0.6,
                    sectionPulse: 0.02,
                    pointerPulse: 0.02,
                    scrollPulse: 0.02,
        ...
        this.broadcastContextKeySet = {
            ...
            hoveredCards: true,
            sectionHue: true,
            sectionSaturation: true,
            sectionLightness: true,
        ...
        this.activeSectionAccent = this.sectionProfiles.hero?.accent || null;
        this.pulses = { section: 0, pointer: 0, scroll: 0, event: 0 };
        this.sectionTransition = { lastSection: 'hero', elapsed: 0 };
        this.state.sectionBeacon = this.buildSectionBeacon('hero', this.activeSectionAccent);
        this.updateDominantColorFromBeacon(this.state.sectionBeacon);
    ...
    transitionToSection(sectionId) {
        ...
        const { accent, ...visualProfile } = profile;
        Object.assign(this.visualTarget, visualProfile);
        this.activeSectionAccent = accent || null;
        this.registerSectionTransition(sectionId, profileKey, accent);
    ...
    registerSectionTransition(sectionId, profileKey, accent) {
        const beacon = this.buildSectionBeacon(sectionId, accent);
        this.state.sectionBeacon = beacon;
        this.updateDominantColorFromBeacon(beacon);
        this.sectionTransition.lastSection = sectionId;
        this.sectionTransition.elapsed = 0;
        this.triggerPulse('section', 1);
        window.dispatchEvent(new CustomEvent('sectionBeacon', {
            detail: {
                section: sectionId,
                profileKey,
                beacon
    ...
    buildSectionBeacon(sectionId, accent) {
        const profileKey = this.getSectionProfileKey(sectionId);
        const profile = this.sectionProfiles[profileKey] || this.sectionProfiles.hero;
        const accentData = accent || profile?.accent || {};
        const hue = Number.isFinite(accentData.hue) ? accentData.hue : (profile?.hue ?? 180);
        const saturation = Number.isFinite(accentData.saturation) ? accentData.saturation : 68;
        const lightness = Number.isFinite(accentData.lightness) ? accentData.lightness : 55;
        const highlight = Number.isFinite(accentData.highlight) ? accentData.highlight : Math.min(100, lightness + 12);
        const shadow = Number.isFinite(accentData.shadow) ? accentData.shadow : Math.max(0, lightness - 18);
        const labelSource = accentData.label || profileKey || sectionId || 'section';
        const label = String(labelSource).replace(/[-_]+/g, ' ').replace(/\b\w/g, (match) => match.toUpperCase());
        ...
        return {
            ...
            highlight: Math.max(0, Math.min(100, highlight)),
            shadow: Math.max(0, Math.min(100, shadow))
        };
    }

    updateDominantColorFromBeacon(beacon) {
        if (!beacon) {
            return;
        }

        const hue = beacon.hue ?? 180;
        const saturation = Math.max(0, Math.min(1, (beacon.saturation ?? 70) / 100));
        const lightness = Math.max(0, Math.min(1, (beacon.lightness ?? 55) / 100));
        const value = lightness + saturation * Math.min(lightness, 1 - lightness);
        const normalizedValue = Math.max(0, Math.min(1, value));
        const saturationValue = normalizedValue === 0
            ? 0
            : Math.max(0, Math.min(1, 2 * (1 - lightness / normalizedValue)));
    ...
    triggerPulse(name, magnitude = 1) {
        if (!this.pulses) {
            this.pulses = { section: 0, pointer: 0, scroll: 0, event: 0 };
        }

        if (!(name in this.pulses)) {
            this.pulses[name] = 0;
        }
        ...
        this.pulses[name] = Math.max(this.pulses[name] || 0, clamped);

        if (name !== 'event') {
            const base = Math.max(
                (this.pulses.section || 0) * 0.75,
                (this.pulses.pointer || 0) * 0.65,
                (this.pulses.scroll || 0) * 0.65
            );
            this.pulses.event = Math.max(this.pulses.event || 0, base);
        }

        this.pulses[name] = Math.max(0, Math.min(1, this.pulses[name]));
        this.pulses.event = Math.max(0, Math.min(1, this.pulses.event));
    }

    updatePulseDecay(deltaTime) {
        if (!this.pulses) {
            return;
        }

        const pointerDecay = Math.exp(-deltaTime / 420);
        const scrollDecay = Math.exp(-deltaTime / 560);
        const sectionDecay = Math.exp(-deltaTime / 820);
        const eventDecay = Math.exp(-deltaTime / 640);

        this.pulses.pointer *= pointerDecay;
        this.pulses.scroll *= scrollDecay;
        this.pulses.section *= sectionDecay;

        const baseEvent = Math.max(
            (this.pulses.section || 0) * 0.75,
            (this.pulses.pointer || 0) * 0.65,
            (this.pulses.scroll || 0) * 0.65
        );

        const currentEvent = Number.isFinite(this.pulses.event) ? this.pulses.event : 0;
        this.pulses.event = baseEvent + (currentEvent - baseEvent) * eventDecay;
        this.pulses.event = Math.max(0, Math.min(1, this.pulses.event));
    }

    applyInteractionPulses() {
        const pointerIntensity = Math.max(0, this.dynamics.mouseVelocity - 0.18) * 0.9;
        if (pointerIntensity > 0.04) {
            this.triggerPulse('pointer', Math.min(1, pointerIntensity));
        }

        const scrollIntensity = Math.max(0, this.dynamics.scrollVelocity - 0.22) * 0.8;
        if (scrollIntensity > 0.04) {
            this.triggerPulse('scroll', Math.min(1, scrollIntensity));
        }

        if (this.state.hoveredCards && this.state.hoveredCards.size > 0) {
            this.triggerPulse('pointer', Math.min(1, 0.4 + this.state.hoveredCards.size * 0.08));
        }

        const energyDelta = Math.max(0, this.state.userEnergy - this.restingState.energy);
        if (energyDelta > 0.12) {
            this.triggerPulse('event', Math.min(1, energyDelta * 1.2));
        }
    }
    ...
    onCardHover(card, isEntering) {
        if (isEntering) {
            ...
            this.triggerPulse('pointer', 0.75);
    ...
    triggerCardEmergence(card) {
        ...
        card.dispatchEvent(new CustomEvent('cardEmerge', {
            detail: {
                ...
                beacon: this.state.sectionBeacon
            }
        }));
    }
    ...
    startOrchestration() {
        const update = (timestamp) => {
            ...
            this.sectionTransition.elapsed += deltaTime;
    ...
    updateDynamics(deltaTime) {
        this.updatePulseDecay(deltaTime);
        ...
        this.applyInteractionPulses();
    }

    calculateMultipliers() {
        const sectionPulse = this.pulses?.section || 0;
        const pointerPulse = this.pulses?.pointer || 0;
        const scrollPulse = this.pulses?.scroll || 0;
        const eventPulse = this.pulses?.event || 0;

        const mouseEnergy = Math.min(1, (this.state.mouseActivity * 0.55) + (this.dynamics.mouseVelocity * 2.2) + pointerPulse * 0.35);
        this.multipliers.mouseActivity = 0.85 + (mouseEnergy * 0.6);

        const scrollMomentum = Math.min(1.1, this.dynamics.scrollVelocity + scrollPulse * 0.45);
        ...
        const hoverEnergy = Math.min(1.4, this.dynamics.hover * 0.45 + sectionPulse * 0.6);
        const hoverAmplifier = 0.85 + sectionDepth * 0.5 + eventPulse * 0.2;
        ...
        this.multipliers.userEnergy = 0.55 + (this.dynamics.energy * 0.9) + eventPulse * 0.2;
        this.multipliers.userEnergy = Math.min(1.6, this.multipliers.userEnergy);
    }
    ...
    applyMultipliers() {
        ...
        const accent = this.activeSectionAccent || profile.accent || null;
        const sectionPulse = this.pulses?.section || 0;
        const pointerPulse = this.pulses?.pointer || 0;
        const scrollPulse = this.pulses?.scroll || 0;
        const eventPulse = this.pulses?.event || 0;
        ...
        const pulseLift = sectionPulse * 0.35 + eventPulse * 0.25;

        // Modulate intensity with energy while preserving base profile balance
        this.visualTarget.intensity = profile.intensity + (energyBoost * 0.6) + sectionLift * 0.4 + pulseLift;

        // Modulate chaos and speed using smoothed activity
        const chaosBoost = Math.min(0.6, (this.dynamics.mouseVelocity * 0.4) + (this.dynamics.hover * 0.15) + pointerPulse * 0.3 + eventPulse * 0.2);
        const depthChaos = sectionDepth * 0.18 + sectionPulse * 0.16;
        this.visualTarget.chaos = profile.chaos + chaosBoost + depthChaos;

        const speedBoost = Math.min(0.8, (this.dynamics.mouseVelocity * 0.5) + (this.dynamics.scrollVelocity * 0.3) + scrollPulse * 0.35 + eventPulse * 0.25);
        const depthSpeed = sectionDepth * 0.25 + sectionPulse * 0.12;
        ...
        const hoverRgb = Math.min(0.0012, this.dynamics.hover * 0.0006 + pointerPulse * 0.0008);
        ...
        const moireFromPulses = sectionPulse * 0.1 + eventPulse * 0.2 + pointerPulse * 0.08 + scrollPulse * 0.12;
        const desiredMoire = profile.moireIntensity + moireFromScroll + moireFromEnergy + moireFromHover + depthMoire + moireFromPulses;
        this.visualTarget.moireIntensity = (desiredMoire * 0.7) + (this.dynamics.moire * 0.3);

        // Hue shifts slightly based on mouse position
        const accentHue = accent?.hue ?? profile.hue ?? 180;
        const baseHue = profile.hue !== undefined ? profile.hue : accentHue;
        const blendedHue = (baseHue * 0.65) + (accentHue * 0.35);
        const hueShift = (this.state.mousePos.x - 0.5) * 40 + pointerPulse * 12 - scrollPulse * 6;
        const depthHue = sectionDepth * 8 + sectionPulse * 6 + eventPulse * 10;
        this.visualTarget.hue = (blendedHue + hueShift + depthHue + 360) % 360;

        if (profile.formMix !== undefined) {
            this.visualTarget.formMix = profile.formMix + sectionDepth * 0.1 + eventPulse * 0.05;
        }
    ...
    composeBroadcastPayload() {
        ...
        const beacon = this.state.sectionBeacon;
        if (beacon) {
            context.sectionHue = beacon.hue;
            context.sectionSaturation = beacon.saturation;
            context.sectionLightness = beacon.lightness;
            context.sectionHighlight = beacon.highlight;
            context.sectionShadow = beacon.shadow;
            context.sectionLabel = beacon.label;
        } else {
            delete context.sectionHue;
            delete context.sectionSaturation;
            delete context.sectionLightness;
            delete context.sectionHighlight;
            delete context.sectionShadow;
            delete context.sectionLabel;
        }

        context.sectionPulse = this.pulses?.section || 0;
        context.pointerPulse = this.pulses?.pointer || 0;
        context.scrollPulse = this.pulses?.scroll || 0;
        context.eventPulse = this.pulses?.event || 0;
        context.sectionElapsed = (this.sectionTransition.elapsed || 0) / 1000;
