...
(function() {
  ...
  function ensureCardVariant(card, variantKey, index) {
    ...
    state.activeNode = state.variant?.node || state.activeNode || 'quantum';
  ...
  function ensureOverlayFrameStore(overlay) {
    if (!overlay.frames) {
      overlay.frames = new Map();
    }

    return overlay.frames;
  }

  function createOverlayFrame(node) {
    const frame = document.createElement('iframe');
    frame.src = '';
    frame.dataset.vib3Hydrated = '0';
    frame.dataset.vib3Active = '0';
    frame.dataset.vib3Node = node || 'quantum';
  ...
  function ensureNodeFrame(overlay, node) {
    if (!overlay) return null;

    const frames = ensureOverlayFrameStore(overlay);
    const targetNode = node || 'quantum';

    if (frames.has(targetNode)) {
      return frames.get(targetNode);
    }

    const frame = createOverlayFrame(targetNode);
    frames.set(targetNode, frame);
    overlay.container.appendChild(frame);
  ...
  function activateFrame(overlay, frame, options = {}) {
    if (!overlay || !frame) return frame;

    const { occlusion = 1, parkOthers = true, unloadOthers = false } = options;
    const previousFrame = overlay.frame && overlay.frame !== frame ? overlay.frame : null;

    if (previousFrame) {
      parkFrame(previousFrame);

      if (unloadOthers) {
        dehydrateFrame(previousFrame);
        previousFrame.dataset.vib3Src = '';
      }
    }

    if (parkOthers && overlay.frames) {
      overlay.frames.forEach((otherFrame) => {
        if (otherFrame === frame || otherFrame === previousFrame) return;
        parkFrame(otherFrame);

        if (unloadOthers) {
          dehydrateFrame(otherFrame);
          otherFrame.dataset.vib3Src = '';
        }
      });
    }

    overlay.frame = frame;
    overlay.activeNode = frame.dataset.vib3Node || 'quantum';
    frame.dataset.vib3Active = '1';
    frame.style.visibility = occlusion < 1 ? 'visible' : 'hidden';
    frame.style.opacity = occlusion < 1 ? '1' : '0';
  ...
  function updateOverlaySource(overlay, variant, options = {}) {
    ...
    const frame = options.frame || overlay.frame;

    if (!frame) return;

    if (!variant) {
      if (!hydrate) {
        frame.dataset.vib3Src = '';
        dehydrateFrame(frame);
      }
      return;
    }
  ...
  function ensureCardOverlay(card, state, options = {}) {
    ...
    if (!overlay) {
      ...
      overlay = {
        ...
        frame: null,
        frames: new Map(),
        activeNode: state?.activeNode || 'quantum',
      };

      cardRegistry.set(card, overlay);
    }

    const occlusion = state?.occlusion ?? 1;
    const node = variant?.node || state?.activeNode || overlay.activeNode || 'quantum';
    const frame = ensureNodeFrame(overlay, node);

    activateFrame(overlay, frame, { occlusion });
    refreshOverlayTheme(overlay, variant);
    updateOverlaySource(overlay, variant, { hydrate, frame });
    frame.style.transform = buildFrameTransform(variant, 0, 0, 0, occlusion);
    applyOcclusion(overlay, state, occlusion);

    state.overlay = overlay;
    state.activeNode = overlay.activeNode || state.activeNode;
  ...
  function prepareNextVariant(card, state) {
    ...
    const occlusion = state.occlusion ?? 1;
    const frame = ensureNodeFrame(overlay, variant.node || overlay.activeNode || 'quantum');

    activateFrame(overlay, frame, { occlusion, unloadOthers: true });
    refreshOverlayTheme(overlay, variant);
    updateOverlaySource(overlay, variant, { hydrate: true, frame });
    frame.style.transform = buildFrameTransform(variant, 0, 0, 0, occlusion);
    frame.style.visibility = 'hidden';
    frame.style.opacity = '0';
    applyOcclusion(overlay, state, occlusion);
    ...
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        ...
        if (overlay.frame) {
          ...
          overlay.frame.style.opacity = state.occlusion === 0 ? '1' : '0';
        }
        return;
      }
      ...
      transitionOcclusion(card, state, overlay, 1, { instant: true }).then(() => {
        ...
        if (overlay.frame) {
          ...
          overlay.frame.style.opacity = '0';
          parkFrame(overlay.frame);
        }

        if (overlay.frames && overlay.frames.size) {
          overlay.frames.forEach((frame) => {
            updateOverlaySource(overlay, null, { hydrate: false, frame });
          });
        } else {
          updateOverlaySource(overlay, null, { hydrate: false });
        }
      });
    });
  }
  ...
  function teardownOverlays() {
    ...
    cardRegistry.forEach((overlay) => {
      const { container, frames } = overlay;
