...
function initPolytopalField(canvas) {
  ...
  const drawNodes = () => {
    ...
    const pointerContribution = pointer.active ? 0.18 : 0;
    const moireStrength = Math.min(
      1.35,
      Math.max(0, 0.28 + (1.1 - stabilityFactor) * 0.65 + morphFactor * 0.55 + pointerContribution)
    );
    const moireAmplitude = 6 + moireStrength * 26;
    ...
    nodes.forEach((node) => {
      ...
      if (moireStrength > 0.05) {
        const overlays = [
          {
            dx: -moireAmplitude,
            dy: moireAmplitude * 0.35,
            color: `rgba(255, 64, 128, ${0.12 + moireStrength * 0.22})`,
            blur: 8
          },
          {
            dx: moireAmplitude * 0.65,
            dy: -moireAmplitude * 0.4,
            color: `rgba(48, 200, 255, ${0.1 + moireStrength * 0.2})`,
            blur: 10
          },
          {
            dx: moireAmplitude * 0.18,
            dy: moireAmplitude * 0.75,
            color: `rgba(120, 96, 255, ${0.08 + moireStrength * 0.18})`,
            blur: 12
          }
        ];

        overlays.forEach(({ dx, dy, color, blur }) => {
          ctx.save();
          ctx.globalCompositeOperation = 'lighter';
          ctx.filter = `blur(${(blur / 6 + moireStrength * 1.8).toFixed(2)}px)`;
          const overlayGradient = ctx.createRadialGradient(
            ...
            node.x + dx,
            node.y + dy,
            node.radius * (8 + moireStrength * 11) * zoomScale
          );
          ...
          ctx.arc(
            node.x + dx,
            node.y + dy,
            node.radius * (3.4 + moireStrength * 2.8) * zoomScale,
            0,
            Math.PI * 2
          );
          ctx.fill();
