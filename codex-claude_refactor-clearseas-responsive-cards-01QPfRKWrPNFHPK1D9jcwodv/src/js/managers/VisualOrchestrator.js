...
export class VisualOrchestrator {
    ...
    setupListeners() {
        ...
        // Listen to Advanced Card System events
        window.addEventListener('cardSystemEvent', ({ detail }) => {
            this.onCardSystemEvent(detail.event, detail.params);
        });

        window.addEventListener('cardLocked', () => {
            this.state.userEnergy = Math.min(1.0, this.state.userEnergy + 0.3);
        });

        window.addEventListener('cardUnlocked', () => {
            this.state.userEnergy *= 0.7;
        });
    }
    ...
    onCardSystemEvent(event, params) {
        console.log(`🎨 VisualOrchestrator responding to: ${event}`, params);

        // Override visual targets based on card interactions
        switch (event) {
            case 'choreography-step':
                // Dynamic choreographed sequence - each step sets new targets
                console.log(`  🎭 Choreography step ${params.step + 1}/${params.total}: ${params.description}`);

                // Apply the step's parameters to visual targets
                if (params.intensity !== undefined) this.visualTarget.intensity = params.intensity;
                if (params.chaos !== undefined) this.visualTarget.chaos = params.chaos;
                if (params.speed !== undefined) this.visualTarget.speed = params.speed;
                if (params.hue !== undefined) this.visualTarget.hue = params.hue;
                if (params.rgbOffset !== undefined) this.visualTarget.rgbOffset = params.rgbOffset;

                // Apply moiré intensity if specified
                if (params.moireIntensity !== undefined) {
                    this.visualTarget.moireIntensity = params.moireIntensity;
                }

                // User energy boost during choreography
                this.state.userEnergy = Math.min(1.0, this.state.userEnergy + 0.15);
                break;

            case 'lock':
                // Legacy fallback for simple lock
                this.visualTarget.intensity = params.intensity || 0.85;
                this.visualTarget.chaos = params.chaos || 0.05;
                this.visualTarget.speed = 0.3;
                if (params.hue) this.visualTarget.hue = params.hue;
                break;

            case 'split':
                // Legacy fallback for simple split
                this.visualTarget.intensity = params.intensity || 1.0;
                this.visualTarget.chaos = params.chaos || 0.6;
                this.visualTarget.speed = params.speed || 1.5;
                this.visualTarget.moireIntensity = 0.4;
                this.visualTarget.rgbOffset = 0.005;
                break;

            case 'takeover':
                // Legacy fallback for simple takeover
                this.visualTarget.intensity = params.intensity || 0.95;
                this.visualTarget.chaos = params.chaos || 0.2;
                this.visualTarget.speed = params.speed || 0.4;
                break;

            case 'unlock':
                // Return to section-based profile
                const profile = this.sectionProfiles[this.state.currentSection];
                if (profile) {
                    Object.assign(this.visualTarget, profile);
                }
                // Energy decay on unlock
                this.state.userEnergy *= 0.6;
                break;
        }
    }
