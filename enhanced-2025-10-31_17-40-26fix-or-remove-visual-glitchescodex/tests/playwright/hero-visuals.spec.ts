...
test.describe('Hero choreography responsiveness', () => {
  test('responds to pointer input and orchestration events', async ({ page }) => {
    ...
    const beforePulse = await page.evaluate(() => {
      const value = getComputedStyle(document.documentElement).getPropertyValue('--visual-event-pulse');
      return Number.parseFloat(value) || 0;
    });
    ...
    await page.evaluate(() => {
      const detail = {
        ...
        context: {
          ...
          sectionHue: 196,
          sectionChroma: 0.72,
          sectionLuminance: 0.58,
          sectionPulse: 0.64,
          sectionRipple: 0.48,
          sectionTimeline: 0.32,
          sectionGlyph: 1,
        },
      };
    ...
    await page.waitForFunction(
      (initial) => {
        const value = Number.parseFloat(
          getComputedStyle(document.documentElement).getPropertyValue('--visual-event-pulse')
        );
        return Number.isFinite(value) && value > initial + 0.01;
      },
      beforePulse,
    ...
    await page.waitForFunction(() => document.body.dataset.visualSection === 'hero', {
      timeout: 5_000,
    });

    const beaconColor = await page.evaluate(() =>
      getComputedStyle(document.documentElement).getPropertyValue('--visual-beacon-color').trim()
    );
    expect(beaconColor).toContain('hsl');

    const sectionLabel = await page.evaluate(() => document.body.dataset.visualSection);
    expect(sectionLabel).toBe('hero');

    const glyphSymbol = await page.evaluate(() => document.body.dataset.visualSectionGlyph || '');
    expect(glyphSymbol.length).toBeGreaterThan(0);
