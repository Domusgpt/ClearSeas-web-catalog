...
(function() {
  ...
  const cardVariantMeta = new WeakMap();
  const cardSweepStates = new WeakMap();
  let activeCard = null;
  let rafId = null;
  ...
  const CONFIG = {
    ...
    sweepRevealDuration: 520,
    sweepHideDuration: 460,
    sweepRevealEasing: 'cubic-bezier(0.22, 1, 0.36, 1)',
    sweepHideEasing: 'cubic-bezier(0.55, 0, 0.1, 1)',
  };

  const BASE_ENGINE_URL = 'https://domusgpt.github.io/vib3-plus-engine/';

  const FULL_CLIP = 'inset(0 0 0 0)';
  const FULL_POLYGON_CLIP = 'polygon(0 0, 100% 0, 100% 100%, 0 100%)';
  ...
  const SWEEP_PRESETS = {
    fromLeft: {
      ...
      hideTo: 'inset(0 0 0 100%)',
    },
    fromRight: {
      key: 'fromRight',
      type: 'inset',
      revealFrom: 'inset(0 0 0 100%)',
      hideTo: 'inset(0 100% 0 0)',
    },
    fromTop: {
      key: 'fromTop',
      type: 'inset',
      revealFrom: 'inset(100% 0 0 0)',
      hideTo: 'inset(0 0 100% 0)',
    },
    fromBottom: {
      key: 'fromBottom',
      type: 'inset',
      revealFrom: 'inset(0 0 100% 0)',
      hideTo: 'inset(100% 0 0 0)',
    },
    diagLtr: {
      key: 'diagLtr',
      type: 'polygon',
      revealFrom: 'polygon(0 0, 0 0, 0 100%, 0 100%)',
      hideTo: 'polygon(100% 0, 100% 0, 100% 100%, 100% 100%)',
    },
    diagRtl: {
      key: 'diagRtl',
      type: 'polygon',
      revealFrom: 'polygon(100% 0, 100% 0, 100% 100%, 100% 100%)',
      hideTo: 'polygon(0 0, 0 0, 0 100%, 0 100%)',
    },
  };

  const QUANTUM_SWEEP_POOL = ['fromLeft', 'fromTop', 'diagLtr'];
  const HOLOGRAPHIC_SWEEP_POOL = ['fromRight', 'fromBottom', 'diagRtl'];
  ...
  function ensureCardVariant(card, variantKey, index) {
    if (cardVariants.has(card)) {
      if (!cardVariantMeta.has(card)) {
        const variants = HOLOGRAM_VARIANTS[variantKey] || [];
        const poolSize = variants.length;
        cardVariantMeta.set(card, {
          variantKey,
          poolSize,
          currentIndex: poolSize > 0 ? index % poolSize : 0,
          nextIndex: poolSize > 0 ? (index + 1) % poolSize : 0,
          cycleCount: 0,
          pendingCycle: false,
        });
      }
  ...
  function ensureSweepState(card) {
    let state = cardSweepStates.get(card);
    ...
    if (!state) {
      ...
      cardSweepStates.set(card, state);
    }

    return state;
  }

  function getSweepPool(variant) {
    return (variant && variant.node === 'holographic') ? HOLOGRAPHIC_SWEEP_POOL : QUANTUM_SWEEP_POOL;
  }

  function resolvePreset(key) {
    return key ? SWEEP_PRESETS[key] : null;
  }

  function assignNextSweepPreset(card, overlay, variant, meta = cardVariantMeta.get(card)) {
    if (!overlay) return null;

    const state = ensureSweepState(card);
    const pool = getSweepPool(variant);

    if (!pool.length) {
      overlay.container.dataset.vib3SweepState = 'primed';
      overlay.container.style.clipPath = FULL_CLIP;
      return null;
    }

    const cycleSeed = meta ? meta.cycleCount || 0 : 0;
    const baseSeed = variant?.id || variant?.geometry || variant?.node || 'variant';
    const indexSeed = `${baseSeed}-${cycleSeed}`;
    let index = deterministicIndex(indexSeed, pool.length);
    let candidateKey = pool[index % pool.length];

    if (candidateKey === state.previousKey) {
      candidateKey = pool[(index + 1) % pool.length];
    }

    const preset = resolvePreset(candidateKey);

    if (!preset) {
      return null;
    }

    state.currentKey = preset.key;
    state.awaitingReveal = true;
    overlay.container.dataset.vib3Sweep = preset.key;
    overlay.container.dataset.vib3SweepState = 'primed';
    overlay.container.style.clipPath = preset.revealFrom;

    return preset;
  }

  function ensureCurrentSweepPreset(card, overlay, variant) {
    const state = ensureSweepState(card);

    if (!state.currentKey || !resolvePreset(state.currentKey)) {
      return assignNextSweepPreset(card, overlay, variant);
    }

    const preset = resolvePreset(state.currentKey);
  ...
  function clearSweepAnimation(card) {
    const state = ensureSweepState(card);
  ...
  function playSweep(card, overlay, direction) {
    if (!overlay) return null;

    const variant = cardVariants.get(card);
    const preset = ensureCurrentSweepPreset(card, overlay, variant);

    if (!preset) return null;

    const state = ensureSweepState(card);

    clearSweepAnimation(card);

    const openClip = preset.type === 'polygon' ? FULL_POLYGON_CLIP : FULL_CLIP;
    const startClip = direction === 'in' ? preset.revealFrom : openClip;
    const endClip = direction === 'in' ? openClip : preset.hideTo;
    const easing = direction === 'in' ? CONFIG.sweepRevealEasing : CONFIG.sweepHideEasing;
    const duration = direction === 'in' ? CONFIG.sweepRevealDuration : CONFIG.sweepHideDuration;
    const sweepState = direction === 'in' ? 'revealing' : 'occluding';
    const finishState = direction === 'in' ? 'revealed' : 'occluded';

    overlay.container.dataset.vib3SweepState = sweepState;

    if (!motionEnabled) {
      overlay.container.style.clipPath = endClip;
      overlay.container.dataset.vib3SweepState = finishState;

      if (direction === 'in') {
        state.previousKey = preset.key;
    ...
    overlay.container.style.clipPath = startClip;

    const animation = overlay.container.animate(
      [
        { clipPath: startClip },
        { clipPath: endClip },
      ],
  ...
  function primeVariantForNextRun(card, overlay) {
    const meta = cardVariantMeta.get(card);
    const currentVariant = cardVariants.get(card);

    if (!meta) {
      assignNextSweepPreset(card, overlay, currentVariant);
      return currentVariant;
    }

    if (meta.pendingCycle) {
      return cardVariants.get(card);
    }

    meta.pendingCycle = true;

    const variants = HOLOGRAM_VARIANTS[meta.variantKey] || [];
    const poolSize = meta.poolSize || variants.length || 0;

    let nextVariant = currentVariant;

    if (poolSize > 1 && variants.length > 0) {
      nextVariant = variants[meta.nextIndex % poolSize];
      meta.currentIndex = meta.nextIndex % poolSize;
      meta.nextIndex = (meta.nextIndex + 1) % poolSize;
      cardVariants.set(card, nextVariant);
    }

    meta.cycleCount = (meta.cycleCount || 0) + 1;

    if (overlay) {
      if (nextVariant !== currentVariant) {
        refreshOverlayTheme(overlay, nextVariant);
        updateOverlaySource(overlay, nextVariant, { hydrate: true });
      }
      overlay.frame.style.transform = buildFrameTransform(nextVariant);
      assignNextSweepPreset(card, overlay, nextVariant, meta);
    }

    meta.pendingCycle = false;
  ...
  function ensureCardOverlay(card, variant, options = {}) {
    ...
    if (overlay) {
      ...
      ensureCurrentSweepPreset(card, overlay, variant);
      return overlay;
    }
    ...
    assignNextSweepPreset(card, overlay, variant);
  ...
  function handleCardEnter(card, event) {
    ...
    overlay.container.style.transform = 'translate3d(0, 0, 0)';
    overlay.container.style.willChange = 'clip-path, opacity, transform';
    ...
    const sweepAnimation = playSweep(card, overlay, 'in');

    if (!sweepAnimation && preset) {
      const openClip = preset.type === 'polygon' ? FULL_POLYGON_CLIP : FULL_CLIP;
      overlay.container.style.clipPath = openClip;
      overlay.container.dataset.vib3SweepState = 'revealed';
    }
  }
  ...
  function handleCardLeave(card) {
    ...
    if (overlay) {
      ...
      const finalize = () => {
        ...
        overlay.container.style.willChange = 'auto';
        overlay.frame.style.visibility = 'hidden';
        const nextVariant = primeVariantForNextRun(card, overlay);

        if (!supportsIntersectionObserver && nextVariant) {
          updateOverlaySource(overlay, nextVariant, { hydrate: false });
        }
      };

      const animation = playSweep(card, overlay, 'out');
