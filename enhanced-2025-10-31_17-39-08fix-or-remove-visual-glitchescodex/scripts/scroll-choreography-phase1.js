(function () {
  ...
  function initialiseChoreography() {
    ...
    const handleVisualStateUpdate = (event) => {
      ...
      const momentumPulse = clamp(context?.momentumPulse ?? 0, 0, 1);
      const interactionPulse = clamp(context?.interactionPulse ?? 0, 0, 1);
      const accentStrength = clamp(context?.sectionAccentStrength ?? 0.4, 0, 1);
      const accentHue = Number.isFinite(context?.sectionAccentHue)
        ? Number(context.sectionAccentHue)
        : null;
      const secondaryHue = Number.isFinite(context?.secondaryAccentHue)
        ? Number(context.secondaryAccentHue)
        : null;
      const milestoneValue = clamp(context?.sectionMilestone ?? 0, 0, 1);

      const depthGain = heroDepth * 0.5 + sectionPulse * 0.18;
      const opacityTarget = clamp(
        state.moireIntensity * 0.5 + energy * 0.11 + depthGain * 0.08 + momentumPulse * 0.06,
        0,
        0.38
      );
      const rotationTarget = clamp(
        (state.chaos - 0.25) * 8 + heroDepth * 1.6 + momentumPulse * 1.2 + sectionPulse * 0.9,
        -4.6,
        4.6
      );
      const scaleTarget = clamp(
        (state.intensity - 0.5) * 0.16 + heroDepth * 0.12 + sectionPulse * 0.1,
        -0.08,
        0.22
      );
      const shiftTarget = clamp(
        (scrollProgress - 0.5) * 48 + scrollVelocity * 9 - heroDepth * 6 + momentumPulse * 8 - sectionPulse * 4,
        -36,
        36
      );
      const driftXTarget = clamp(
        (mouseActivity - 0.5) * 40 + hoveredCards * 2.8 + interactionPulse * 6,
        -32,
        34
      );
      const driftYTarget = clamp(
        (energy - 0.5) * -36 - heroDepth * 12 - sectionPulse * 10,
        -32,
        24
      );

      if (accentHue !== null) {
        rootStyle.set('--section-accent-h', `${accentHue}`);
      }
      if (secondaryHue !== null) {
        rootStyle.set('--section-accent-secondary-h', `${secondaryHue}`);
      }
      rootStyle.set('--section-accent-strength', accentStrength.toFixed(3));
      rootStyle.set('--section-pulse', sectionPulse.toFixed(3));
      rootStyle.set('--momentum-pulse', momentumPulse.toFixed(3));
      rootStyle.set('--interaction-pulse', interactionPulse.toFixed(3));
      rootStyle.set('--section-milestone', milestoneValue.toFixed(3));
    ...
    const heroTimeline = gsap.timeline({
      ...
      scrollTrigger: {
        ...
        onToggle(self) {
          heroSceneActive = Boolean(self?.isActive);
          if (!heroSceneActive) {
            resetPointer();
          }
        }
    ...
    heroSceneActive = Boolean(heroTimeline.scrollTrigger?.isActive);
